...
from app.services.utils.voice_enum import voice_cosy, voice_azure
from app.utils import utils


def get_all_azure_voices(filter_locals=None, tts_provider=None) -> list[str]:
    if filter_locals is None:
        filter_locals = ["zh-CN", "en-US", "zh-HK", "zh-TW", "vi-VN"]
    if tts_provider == 'FunAudioLLM/CosyVoice2-0.5B':
        all_voice = voice_cosy
    else:
        all_voice = voice_azure
    voices = []
    for name, gender in all_voice.items():
        # 如果是默认的 tts_provider 并且有过滤条件，则进行过滤
        if tts_provider == 'default':
            if filter_locals:
                for filter_local in filter_locals:
                    if name.lower().startswith(filter_local.lower()):
                        voices.append(f"{name}-{gender.value}")
                        break
            else:
                voices.append(f"{name}-{gender.value}")
        else:
...
def is_azure_v2_voice(voice_name: str):
    ...
    if voice_name in voice_azure and voice_name.endswith("-V2"):
        return voice_name.replace("-V2", "").strip()
    return ""

def is_cosy_voice(voice_name: str):
    voice_name = parse_voice_name(voice_name)
    if voice_name in voice_cosy:
        return voice_name
    return ""


def cosy_tts(text: str, voice_name: str, voice_rate: float, voice_file: str) -> Union[bool, None]:
    cosy_url = config.cosy.get("cosy_url")
    cosy_key = config.cosy.get("cosy_key")
    # 解析语音名称和文本预处理
    voice_name = parse_voice_name(voice_name)
    text = text.strip()
    # 构建请求体
    ...
    headers = {
        "Authorization": f"Bearer {cosy_key}",
        "Content-Type": "application/json"
    }
    for i in range(3):
        try:
            logger.info(f"start, voice name: {voice_name}, try: {i + 1}")
            response = requests.post(cosy_url, json=payload, headers=headers)
            # 检查响应状态码
            if response.status_code == 200:
                ...
                subs, offsets = calculate_time_offsets(sentences)
                sub_maker = SubMaker()
                sub_maker.subs = [''.join(subs)]  # 将所有字符拼接成一个字符串
                sub_maker.offset = offsets
                return sub_maker
            else:
                logger.warning(f"failed, status code: {response.status_code}, response: {response.text}")
                continue
...
def calculate_time_offsets(sentences: List[str], base_offset: float = 0.0) -> Tuple[
    ...
    for sentence in sentences:
        ...
        for word in words:
            for char in word:
                ...
                current_offset += 0.4  # 每个字符持续时间为0.4秒
...
def tts(
    ...
    elif is_cosy_voice(voice_name):
        return cosy_tts(text, voice_name, voice_rate, voice_file)
...
def get_audio_duration(file_path: str):
    ...
    获取音频时长
    """
    with AudioFileClip(file_path) as video_clip:
        duration = video_clip.duration  # 获取音频文件的时长（秒）
    return duration


if __name__ == "__main__":
    duration = get_audio_duration(r"C:\workspace\MoneyPrinterTurbo\storage\tasks\d3823455-2229-4772-81b6-a03498b88918\audio.mp3")
